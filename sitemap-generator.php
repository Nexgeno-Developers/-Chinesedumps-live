...
*************************************************************/
	...
	function GetUrl ($url) {
		$agent = "Mozilla/5.0 (compatible; ChineseDumps PHP XML Sitemap Generator/" . VERSION . ", https://www.chinesedumps.com)";
				...
				if (!$ignore) {
					foreach ($extension as $ext) {
						if (strpos ($next_url, $ext) > 0) {
							...
							if(!in_array( htmlentities ($next_url),$existing_array ))
							{
							$existing_array[] =  htmlentities ($next_url);
							fwrite ($pf, "  <url>\n" .
	...
	fwrite ($pf, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" .
				 "<?xml-stylesheet type=\"text/xsl\" href=\"<?php echo BASE_URL; ?>xml-sitemap.xsl\"?>\n" .
				 "<!-- Created with ChineseDumps PHP XML Sitemap Generator " . VERSION . " https://www.chinesedumps.com -->\n" .
